	...
	If	Subs(aImpressao[nX,1],10) =="0000"                                                                                              //      +-----.T.  server
		If	ascan(xaArq,aImpressao[nX,1]) == 0                                                                                             //      V
			...
			Else
				...
				If	Empty(cModelo)
					cModelo:="ALLEGRO"
					MSCBPRINTER(cModelo,Alltrim(cPorta)+":"+Alltrim(cSettings),,,.F.,,,,,,,)
					MSCBCHKSTATUS(.f.)
				Else
					MSCBPRINTER(cModelo,Alltrim(cPorta)+":"+Alltrim(cSettings),,,.F.,,,,,,,)
					MSCBCHKSTATUS(.t.)
...
If MSCbModelo('ZPL',cImpPar)
	cPrg := 'ZPL'
ElseIf MSCbModelo('DPL',cImpPar)
	cPrg := 'DPL'
ElseIf MSCbModelo('EPL',cImpPar)
	cPrg := 'EPL'
ElseIf MSCbModelo('IPL',cImpPar)
	cPrg := 'IPL'
ElseIf MSCbModelo('TXT',cImpPar)
	cPrg := 'TXT'
EndIf
cPrgAux := cPrg
